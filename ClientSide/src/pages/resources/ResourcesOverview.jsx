  ...
  const handleExport = async () => {
    try {
      ...
      const formattedData = exportData.map(d => ({
        ...
        'Chất đốt': Math.round(d['Chất đốt']),
        'Nước': Math.round(d['Nước']),
        'Vải': Math.round(d['Vải']),
        'Hóa chất': Math.round(d['Hóa chất']),
        'Tái chế': Math.round(d['Tái chế']),
      }));

      await exportToStyledExcel({
        sheets: [{ sheetName: 'Tai_Nguyen', data: formattedData }],
        fileName: `Tong_hop_tai_nguyen_${dayjs().format('DDMMYYYY')}.xlsx`,
      });
    } catch (err) {
      console.error(err);
    ...
    <div className="grid gap-3 p-3 pl-3 h-screen">
      ...
      <div className="col-span-full rounded flex justify-between items-center">
        <div className="Title flex flex-col justify-center">
          <h1 className="PageName text-2xl font-semibold">Tài nguyên</h1>
        </div>
