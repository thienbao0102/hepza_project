...
const AdminSolidWastes = () => {
    ...
    const columns = [
        { Header: "Tên rác thải", accessor: "name", render: (val) => <span className="truncate block w-full text-center font-medium text-slate-700 capitalize" title={val}>{val}</span> },
        ...
        { Header: "Loại rác", accessor: "groupName", render: (val) => <span className={clsx("px-2.5 py-1 rounded-full text-xs font-medium border truncate block w-fit mx-auto capitalize shadow-sm", getGroupStyles(val))} title={val}>{val}</span> },
        ...
        { Header: "Ngày ghi nhận", accessor: "createdAt", render: (val) => <span className="truncate block w-full text-center text-slate-400 font-medium">{formatToVietnamTime(val)}</span> },
    ];
    ...
    const rawData = useMemo(() => solidWasteItems.map(item => ({ ...item, name: item.wasteName || item.name || "Chưa đặt tên", groupName: normalizeGroupName(item.main_group || item.subGroup || item.sub_group), _id: item._id || item.id || `temp-${Math.random()}` })), [solidWasteItems]);
    ...
    const filteredData = useMemo(() => {
        let result = rawData.filter(item => {
            if (searchTerm) { const s = searchTerm.toLowerCase(); if (!(item.name || "").toLowerCase().includes(s) && !(item.groupName || "").toLowerCase().includes(s)) return false; }
            if (selectedFilters.groupName?.length > 0 && !selectedFilters.groupName.includes(item.groupName)) return false;
            return true;
        });
    ...
    const chartData = useMemo(() => {
        ...
        rawData.forEach(item => {
            const group = item.groupName || "Khác"; const qty = Number(item.quantity) || 0; const name = item.name || "Khác";
            groupMap.set(group, (groupMap.get(group) || 0) + qty); nameMap.set(name, (nameMap.get(name) || 0) + qty);
            if (!nameByGroup.has(group)) nameByGroup.set(group, new Map());
            nameByGroup.get(group).set(name, (nameByGroup.get(group).get(name) || 0) + qty);
            const d = dayjs(item.createdAt); const mk = d.isValid() ? d.format("MM/YYYY") : "N/A"; const ts = d.isValid() ? d.valueOf() : 0;
            if (mk !== "N/A") { if (!timeMap.has(mk)) timeMap.set(mk, { name: mk, value: 0, timestamp: ts }); timeMap.get(mk).value += qty; }
        });
        ...
        const byNameGrouped = {};
        nameByGroup.forEach((nMap, grpKey) => { byNameGrouped[grpKey] = Array.from(nMap.entries()).map(([n, v]) => ({ name: n, value: v })).sort((a, b) => b.value - a.value).slice(0, 10); });
...
export default AdminSolidWastes;
