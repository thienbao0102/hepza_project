...
const AdminWastewater = () => {
    const COLORS = ['#06B6D4', '#0EA5E9', '#3B82F6', '#6366F1', '#8B5CF6'];

    const columns = [
        { Header: "Loại nước thải", accessor: "name", render: (val) => <span className="font-medium text-slate-700 block truncate capitalize" title={val}>{val}</span> },
        { Header: "Lưu lượng", accessor: "quantity", render: (val) => <span className="font-mono text-blue-600 font-bold block text-center">{val?.toLocaleString()}</span> },
    ...
    const wastewaterItems = useMemo(() => {
        const keywords = ['wwa', 'wa', 'nước thải', 'wastewater', 'nước thải công nghiệp', 'nước thải sinh hoạt'];
        return apiData.filter(item => {
            const sg = (item.main_group || item.subGroup || item.sub_group || '').toLowerCase();
            const name = (item.wasteName || item.name || '').toLowerCase();
            return keywords.includes(sg) || name.includes('nước thải');
        });
    }, [apiData]);

    const rawData = useMemo(() => wastewaterItems.map(item => ({ ...item, name: item.wasteName || item.name || "Chưa đặt tên", treatmentMethods: item.treatmentMethods || item.purpose || "", _id: item._id || item.id || `temp-${Math.random()}` })), [wastewaterItems]);

    const filterOptions = useMemo(() => ({ date_range: [] }), []);
    const fieldLabels = { date_range: "Ngày tạo" };

    const filteredData = useMemo(() => {
        let result = rawData.filter(item => { if (searchTerm) { const s = searchTerm.toLowerCase(); if (!(item.name || "").toLowerCase().includes(s) && !(item.treatmentMethods || "").toLowerCase().includes(s)) return false; } return true; });
    ...
    const CustomAreaTooltip = ({ active, payload, label }) => {
        if (active && payload?.length) {
            ...
            return (<div className="bg-white p-3 border border-slate-100 shadow-xl rounded-xl ring-1 ring-black/5"><p className="mb-2 font-semibold text-slate-700 text-sm border-b pb-1 border-slate-100">{label}</p><div className="flex flex-col gap-1.5">{sorted.map((e, i) => <div key={i} className="flex items-center justify-between gap-4 text-xs"><div className="flex items-center gap-1.5 min-w-[80px]"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: e.color }} /><span className="text-slate-600 truncate max-w-[120px]">{e.name}</span></div><span className="font-mono font-medium text-slate-800">{e.value.toLocaleString()} <span className="text-slate-400 text-[10px]">m³</span></span></div>)}<div className="pt-2 mt-1 border-t border-slate-100 flex justify-between items-center text-xs font-semibold text-slate-800"><span>Tổng cộng</span><span>{sorted.reduce((a, b) => a + b.value, 0).toLocaleString()} m³</span></div></div></div>);
        }
        return null;
    };
    ...
    useEffect(() => {
        ...
        setBreadcrumbItems([{ key: '/admin/waste', title: "Quản lý Chất thải" }, { key: '/admin/waste/wastewater', title: "Nước thải" }]);
...
export default AdminWastewater;
