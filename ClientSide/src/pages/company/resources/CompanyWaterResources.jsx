...
import { Droplet, SearchCheck, Trash, TrendingUp, PieChart as PieChartIcon, BarChart3, Maximize2 } from 'lucide-react';
...
const CompanyWaterResources = () => {
  // Styles for Water Types (Source)
  const getSourceStyles = (source) => {
    const lower = (source || '').toLowerCase();
    if (lower.includes('cấp')) return 'bg-cyan-50 text-cyan-700 border-cyan-200';
    if (lower.includes('thải')) return 'bg-amber-50 text-amber-700 border-amber-200';
    if (lower.includes('mưa')) return 'bg-indigo-50 text-indigo-700 border-indigo-200';
    if (lower.includes('tái sử dụng') || lower.includes('tái chế')) return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  ...
  const getPurposeStyles = (purpose) => {
    ...
    if (lower.includes('sinh hoạt') || lower.includes('domestic')) return 'bg-teal-50 text-teal-700 border-teal-200';
    if (lower.includes('khác') || lower.includes('other')) return 'bg-gray-50 text-gray-600 border-gray-200';
    return 'bg-slate-50 text-slate-700 border-slate-200';
  };

  const columns = [
    {
      Header: 'Loại nước (Nguồn)',
      accessor: 'source',
      render: (val) => {
  ...
  const { data: summaryRecords = [], isFetching, refetch } = useSummaryDetail(summaryParams, {
    ...
    keepPreviousData: false, // Prevent stale data
  });
  ...
  const rawData = useMemo(() => {
    ...
    apiData.forEach(item => {
      ...
      let sourceName = (item.fuelName || 'Khác').trim();
      const lower = sourceName.toLowerCase();
      if (lower.includes('giếng')) sourceName = 'Nước Giếng';
      else if (lower.includes('cấp')) sourceName = 'Nước Cấp';
      else if (lower.includes('tái chế') || lower.includes('tái sử dụng')) sourceName = 'Nước Tái Chế';
      else if (lower.includes('mưa')) sourceName = 'Nước Mưa';
  ...
  const chartData = useMemo(() => {
    ...
    // Use ORIGINAL apiData for time trend to avoid double-counting
    // AND aggregate by source for Stacked Bar Chart
    const allSources = new Set();
    apiData.forEach(item => {
      const qty = Number(item.quantity) || 0;

      // Normalize source name for charts too
      let source = (item.fuelName || 'Khác').trim();
      const lower = source.toLowerCase();
      if (lower.includes('giếng')) source = 'Nước Giếng';
      else if (lower.includes('cấp')) source = 'Nước Cấp';
      else if (lower.includes('tái chế') || lower.includes('tái sử dụng')) source = 'Nước Tái Chế';
      else if (lower.includes('mưa')) source = 'Nước Mưa';
  ...
  useEffect(() => {
    ...
    const isManager = userRole === 'manager';
    setHeaderConfig({
      title: isAdmin ? 'Quản lý Nước tất cả khu công nghiệp' : (isManager ? 'Quản lý Nước của khu công nghiệp' : 'Quản lý Nước'),
      description: isAdmin ? 'Thống kê và quản lý tài nguyên nước toàn thành phố' : (isManager ? 'Thống kê và quản lý tài nguyên nước khu công nghiệp' : 'Thống kê và quản lý tài nguyên nước'),
      showWeather: true,
      showDatePicker: true,
    });
    setBreadcrumbItems([
  ...
  useEffect(() => {
    ...
    setHeaderConfig({
      ...
      description: `Thống kê và quản lý tài nguyên nước của ${managerZoneLabel}`,
      showWeather: true,
      showDatePicker: true,
    });
    setBreadcrumbItems([
      { key: '/manager/resources', title: `Tài nguyên` },
      { key: '/manager/resources/waterResources', title: `Nước` },
    ]);
    ...
    <div className="flex flex-col h-full overflow-hidden bg-slate-50/50">
      ...
      <div className="min-h-[200px] h-[28%] shrink-0">
        <ResourceAnalytics
          ...
          exportFileName="ThongKeCompanyWaterResources"
        ...
        <div className="flex flex-wrap gap-3 justify-between items-center pb-4 pt-5 border-b border-slate-100">
          ...
          <div className="flex flex-1 gap-2 justify-end items-center max-w-2xl">
            ...
            <div className="w-full max-w-xs">
              <SearchBox
                placeholder="Tìm kiếm loại nước..."
...
export default CompanyWaterResources;
