...
const CompanyEmissions = () => {
  // Air/Sky Theme Colors (Slate -> Sky -> Violet)
  ...
  const getNameStyles = (name) => {
    ...
    if (lower.includes('so2') || lower.includes('nox') || lower.includes('co')) return 'bg-rose-50 text-rose-700 border-rose-200';
    if (lower.includes('bụi') || lower.includes('dust')) return 'bg-amber-50 text-amber-700 border-amber-200';
    if (lower.includes('hơi') || lower.includes('hữu cơ')) return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  ...
  const columns = [
    ...
    },
    {
      Header: 'Lưu lượng',
      accessor: 'quantity',
      render: (val) => <span className="font-mono text-sky-600 font-bold block text-center" title={val?.toLocaleString()}>{val?.toLocaleString()}</span>,
    },
    {
      Header: 'Đơn vị',
      accessor: 'unit',
  ...
  const summaryParams = {
    ...
    include: [6], // Confirmed Waste data group
  };
  ...
  // Filter relevant emissions subgroups
  const emissionsItems = useMemo(() => {
    const emissionsKeywords = ['khí thải', 'gas', 'emission', 'khói', 'bụi', 'hơi'];
    return apiData.filter(item => {
      const sg = (item.main_group || item.subGroup || item.sub_group || '').toLowerCase();
      const name = (item.wasteName || item.name || '').toLowerCase();
      return emissionsKeywords.some(k => sg.includes(k) || name.includes(k));
    });
  }, [apiData]);

  // Process data for table
  const rawData = useMemo(() => {
    return emissionsItems.map(item => ({
      ...item,
      name: item.wasteName || item.name || 'Chưa đặt tên',
      unit: 'mg/l', // Default unit for gas
  ...
  useEffect(() => {
    ...
    const isManager = userRole === 'manager';
    setHeaderConfig({
      title: isAdmin ? 'Dữ liệu chi tiết Khí thải tất cả khu công nghiệp' : (isManager ? 'Dữ liệu chi tiết Khí thải của khu công nghiệp' : 'Dữ liệu chi tiết Khí thải'),
      description: isAdmin ? 'Thông tin chi tiết về khí thải toàn thành phố' : (isManager ? 'Thông tin chi tiết về khí thải khu công nghiệp' : 'Thông tin chi tiết về khí thải phát sinh'),
      showWeather: true,
      showDatePicker: true,
    });
    setBreadcrumbItems([
      { key: '/waste', title: 'Quản lý Chất thải' },
      { key: '/waste/gas-waste', title: 'Khí thải' },
    ]);
  ...
  useEffect(() => {
    ...
    setHeaderConfig({
      ...
      description: `Thông tin chi tiết về khí thải của ${managerZoneLabel}`,
      showWeather: true,
      showDatePicker: true,
    });
    setBreadcrumbItems([
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-slate-100 shadow-xl rounded-xl ring-1 ring-black/5">
          <p className="mb-2 font-semibold text-slate-700 text-sm border-b pb-1 border-slate-100">{label}</p>
            ...
            {sortedPayload.map((entry, index) => (
              <div key={index} className="flex items-center justify-between gap-4 text-xs">
                ...
                <span className="font-mono font-medium text-slate-800">
                  {entry.value.toLocaleString()} <span className="text-slate-400 text-[10px] font-normal">mg/l</span>
                </span>
              </div>
            ))}
            <div className="pt-2 mt-1 border-t border-slate-100 flex justify-between items-center text-xs font-semibold text-slate-800">
              <span>Tổng cộng</span>
              <span>{sortedPayload.reduce((a, b) => a + b.value, 0).toLocaleString()} mg/l</span>
            </div>
          </div>
        </div>
      );
    ...
    <div className="flex flex-col h-full overflow-hidden bg-slate-50/50">
      ...
      <div className="min-h-[200px] h-[28%] shrink-0">
        <ResourceAnalytics
          ...
          exportFileName="ThongKeCompanyEmissions"
