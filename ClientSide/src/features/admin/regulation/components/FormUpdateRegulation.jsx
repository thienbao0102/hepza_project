...
const FormUpdateRegulation = () => {
    const navigate = useNavigate();
    const { regulationId } = useParams();
    const { setHeaderConfig, setBreadcrumbItems } = useHeader();
    const { regulations, editRegulation, loading, fetchRegulations } = useRegulation();
    ...
    // Find current regulation from list
    const regulation = useMemo(() =>
        regulations.find(r => String(r.id) === String(regulationId)),
    ...
    }, [fetchRegulations, regulations.length]);

    useEffect(() => {
        if (regulation && !isInitialized) {
            setFormData({
                title: regulation.title || '',
                group: regulation.group_regulation || regulation.group || 'Nghị Định',
                summary: regulation.summary || '',
                effectiveDate: regulation.effectiveDate ? dayjs(regulation.effectiveDate) : null,
                link: regulation.link || '',
                __v: regulation.__v ?? null
            });
            setIsInitialized(true);
        }
    }, [regulation, isInitialized]);

    useEffect(() => {
        setHeaderConfig({
            title: 'Chỉnh sửa nghị định',
            description: regulation ? `Đang chỉnh sửa: ${regulation.title}` : 'Đang tải thông tin...',
            showWeather: true,
        ...
        setBreadcrumbItems([
            ...
            { key: `/admin/regulations/${regulationId}/edit`, title: "Chỉnh sửa" },
        ]);
    }, [setHeaderConfig, setBreadcrumbItems, regulation, regulationId]);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await editRegulation(regulationId, payload);
            if (result.success) {
                toast.success('Thành công', 'Cập nhật nghị định thành công');
                setTimeout(() => {
                    navigate('/admin/solutions', { state: { activeTab: 'decrees' } });
                }, 1000);
            } else {
                const { title, description } = mapErrorToNotification(result, 'COMMON');
                toast.error(title ?? 'Có lỗi xảy ra khi cập nhật nghị định', description ?? (result.error || 'Có lỗi xảy ra khi cập nhật nghị định'));
            }
        } catch (error) {
    ...
    const renderStepContent = () => {
        if (!regulation && regulations.length > 0 && !loading) {
            return (
                <div className="flex flex-col items-center justify-center h-64 text-slate-400">
                    <AlertCircle size={48} className="mb-4 opacity-20" />
                    <p>Không tìm thấy thông tin nghị định</p>
                    <Button onClick={() => navigate('/admin/solutions')} className="mt-4">Quay lại danh sách</Button>
                </div>
            );
        }
        ...
        if (!isInitialized && loading) {
            return (
                <div className="flex flex-col items-center justify-center h-64">
                    <div className="w-12 h-12 border-4 border-[#4E5BA6]/20 border-t-[#4E5BA6] rounded-full animate-spin" />
                    <p className="mt-4 text-[#4E5BA6] font-medium">Đang tải dữ liệu...</p>
                </div>
            );
        }
                ...
                return (
                    ...
                    >
                        ...
                        <div className="p-5 rounded-2xl bg-[#4E5BA6]/5 border border-[#4E5BA6]/20 flex gap-4 items-start mt-4">
                            ...
                            <div>
                                <h4 className="font-bold text-[#4E5BA6] mb-1 text-base">Cập nhật thông tin</h4>
                                <p className="text-sm text-gray-600 leading-relaxed">
                                    Bạn đang thay đổi thông tin của nghị định hiện tại. Đảm bảo các thông tin mới là chính xác nhất trước khi lưu lại.
                                </p>
                            </div>
                        </div>
                    </motion.div>
...
export default FormUpdateRegulation;
